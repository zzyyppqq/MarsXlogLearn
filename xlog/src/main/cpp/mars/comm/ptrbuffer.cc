...
void PtrBuffer::Write(const void* _pBuffer, size_t _nLen, off_t _nPos) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "PtrBuffer::Write() ptr: %p, _nLen: %zu, _nPos: %ld, length_: %zu", Ptr(), _nLen, _nPos, length_);
}
...
void PtrBuffer::Length(off_t _nPos, size_t _nLenght) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "PtrBuffer::Length() ptr: %p, _nPos: %ld, _nLenght: %zu, max_length_: %zu, length_: %zu", Ptr(), _nPos, _nLenght, max_length_, length_);
}
...
size_t PtrBuffer::Length() const {
    //__android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "PtrBuffer::Length get buff parray_: %p, length_: %d", Ptr(), length_);
    return length_;
}
...
void PtrBuffer::Attach(void* _pBuffer, size_t _nLen, size_t _maxlen) {
    ...
    length_ = _nLen;
    max_length_ = _maxlen;
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "PtrBuffer::Attach() length_: %zu, max_length_: %zu, parray_: %p", length_, max_length_, parray_);
}
...
void PtrBuffer::Reset() {
    ...
    length_ = 0;
    max_length_ = 0;
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "PtrBuffer::Reset() reset");
}
