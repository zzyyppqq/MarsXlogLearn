...
void log_formater(const XLoggerInfo* _info, const char* _logbody, PtrBuffer& _log) {
    //LOGI("zyp-xlog", "_info: %p, _logbody: %s, PtrBuffer _log: %d", &_info, _logbody, _log.Length());
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "log_formater _info: [%p], _logbody: [%s], PtrBuffer _log len: %zu", &_info, _logbody, _log.Length());
