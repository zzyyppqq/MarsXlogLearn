...
void LogBaseBuffer::Flush(AutoBuffer& _buff) {
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "LogBaseBuffer::Flush (__Flush() and __Clear()) ptr: %p", buff_.Ptr());
    ...
    __Flush();
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "LogBaseBuffer::Flush (__Flush() and __Clear()) ptr: %p, log cache: [%s], len: %zu", buff_.Ptr(), (char*)buff_.Ptr(), buff_.Length());
    std::string log_cache_buffer;
    log_cache_buffer.assign((char*)buff_.Ptr(), buff_.Length());
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "LogBaseBuffer::Flush() log cache: [%s], size: %zu, len: %zu", log_cache_buffer.c_str(), log_cache_buffer.size(), log_cache_buffer.length());
...
bool LogBaseBuffer::Write(const void* _data, size_t _inputlen, AutoBuffer& _out_buff) {
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Write _data: [%s], _inputlen: %zu, _out_buff: %zu", (char *)_data, _inputlen, _out_buff.Length());
...
bool LogBaseBuffer::Write(const void* _data, size_t _length) {
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Write1  ptr: %p, len: %zu, log len: %zu, log data: [%s]", buff_.Ptr(), buff_.Length(), _length, (char *)_data);
    ...
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Write2 start compress is_compress_: %d", is_compress_);
    if (is_compress_) {
    ...
    compress_buffer.assign((char*)buff_.Ptr() + before_len, write_len + remain_nocrypt_len_);
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Write3 compress log data: [%s]", compress_buffer.c_str());

    before_len -= remain_nocrypt_len_;

    std::string out_buffer;
    size_t last_remain_len = remain_nocrypt_len_;
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Write4 start crypt");
    log_crypt_->CryptAsyncLog((char*)buff_.Ptr() + before_len,
                              write_len + remain_nocrypt_len_,
                              out_buffer,
                              remain_nocrypt_len_);

    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Write5 crypt before before_len: %zu, ptr: %p, log size: %zu, log data : [%s]", before_len, buff_.Ptr(), out_buffer.size(), out_buffer.data());
    buff_.Write(out_buffer.data(), out_buffer.size(), before_len);
    before_len += out_buffer.size();
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Write6 buff_.Length() before_len: %zu, prt: %p", before_len, buff_.Ptr());
...
bool LogBaseBuffer::__Reset() {
    ...
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::__Reset() buff_.Length() %d, prt: %p", log_crypt_->GetHeaderLen(), buff_.Ptr());
...
void LogBaseBuffer::__Flush() {
    ...
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::__Flush() buff_.Length() %zu, prt: %p", buff_.Length() + log_crypt_->GetTailerLen(), buff_.Ptr());
...
void LogBaseBuffer::__Clear() {
    ...
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::__Clear() buff_.Length(0, 0) prt: %p", buff_.Ptr());
    buff_.Length(0, 0);
...
void LogBaseBuffer::__Fix() {
    ...
    if (log_crypt_->Fix((char*) buff_.Ptr(), buff_.Length(), raw_log_len)) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::Wr__Fix() buff_.Length() %d, prt: %p", raw_log_len + log_crypt_->GetHeaderLen(), buff_.Ptr());
        buff_.Length(raw_log_len + log_crypt_->GetHeaderLen(), raw_log_len + log_crypt_->GetHeaderLen());
    } else {
        __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "LogBaseBuffer::__Fix() buff_.Length(0, 0) prt: %p", buff_.Ptr());
        buff_.Length(0, 0);
    }
}
