...
static const unsigned int kBufferBlockLength = 10 * 1024;
...
void XloggerAppender::SetMode(TAppenderMode _mode) {
    config_.mode_ = _mode;
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::SetMode() notifyAll()");
...
void XloggerAppender::Flush() {
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::Flush() notifyAll()");
    cond_buffer_async_.notifyAll();
}
...
void XloggerAppender::FlushSync() {
    ...
    AutoBuffer tmp;
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::FlushSync()");
    log_buff_->Flush(tmp);
...
void XloggerAppender::Close() {
    ...
    log_close_ = true;
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::Close() notifyAll()");
    cond_buffer_async_.notifyAll();
...
void XloggerAppender::Open(const XLogConfig& _config) {
    ...
    if (OpenMmapFile(mmap_file_path, kBufferBlockLength, mmap_file_)) {
        if (_config.compress_mode_ == kZstd) {
            ...
            log_buff_ = new LogZlibBuffer(mmap_file_.data(), kBufferBlockLength, IS_COMPRESS, _config.pub_key_.c_str());
        }
        use_mmap = true;
    } else {
        ...
        if (_config.compress_mode_ == kZstd) {
            log_buff_ =
                new LogZstdBuffer(buffer, kBufferBlockLength, IS_COMPRESS, _config.pub_key_.c_str(), _config.compress_level_);
        } else {
            log_buff_ = new LogZlibBuffer(buffer, kBufferBlockLength, IS_COMPRESS, _config.pub_key_.c_str());
        }
        use_mmap = false;
    }
    ...
    AutoBuffer buffer;
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::Open()");
    ...
    char mark_info[512] = {0};
    __GetMarkInfo(mark_info, sizeof(mark_info));
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::Open() mark_info1: %s", mark_info);
    ...
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::Open() mark_info2: %s", mark_info);
    ...
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::Open() appender_info: %s", appender_info);
...
bool XloggerAppender::__AppendFile(const std::string& _src_file, const std::string& _dst_file) {
    ...
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::__AppendFile _src_file: %s, _dst_file: %s", _src_file.c_str(), _dst_file.c_str());
...
bool XloggerAppender::__WriteFile(const void* _data, size_t _len, FILE* _file) {
    ...
    std::string log_cache_buffer;
    log_cache_buffer.assign((char *) _data, _len);
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::__WriteFile _data: [%s], data size: %zu, _len: %zu", log_cache_buffer.c_str(), log_cache_buffer.length(), _len);
...
bool XloggerAppender::__OpenLogFile(const std::string& _log_dir) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::__OpenLogFile logfilepath: [%s]", logfilepath);
    if (nullptr == logfile_) {
...
void XloggerAppender::__Log2File(const void* _data, size_t _len, bool _move_file) {
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::__Log2File _data: [%s], _len: %zu, _move_file: %d", (char*)_data, _len, _move_file);
    ...
    if (config_.cachedir_.empty()) {
        __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::__Log2File __OpenLogFile logdir_: [%s]", config_.logdir_.c_str());
...
void XloggerAppender::WriteTips2File(const char* _tips_format, ...) {
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::WriteTips2File _tips_format: [%s]", _tips_format);
    if (nullptr == _tips_format) {
        return;
    }
...
void XloggerAppender::__AsyncLogThread() {
    __android_log_print(ANDROID_LOG_ERROR, "zyp-xlog","XloggerAppender::__AsyncLogThread() while (true)");
    while (true) {
        ScopedLock lock_buffer(mutex_buffer_async_);
        ...
        AutoBuffer tmp;
        __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::__AsyncLogThread() Flush");
        log_buff_->Flush(tmp);
        lock_buffer.unlock();

        if (tmp.Length() > 3000) {
            std::string log_cache_buffer;
            log_cache_buffer.assign((char *) tmp.Ptr(), tmp.Length());
            log_cache_buffer.insert(0, "R\"(");
            log_cache_buffer.append(")\"");
            std::string wrapper = "R\"()\"";
            __android_log_print(ANDROID_LOG_WARN, "zyp-xlog",
                                "XloggerAppender::__Log2File() log cache: [%s], len: %zu",
                                log_cache_buffer.c_str(), tmp.Length() + wrapper.length());
        }
        if (nullptr != tmp.Ptr())
            __Log2File(tmp.Ptr(), tmp.Length(), true);

        if (log_close_) {
            __android_log_print(ANDROID_LOG_ERROR, "zyp-xlog","XloggerAppender::__AsyncLogThread() close");
            break;
        }
        __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::__AsyncLogThread() cond_buffer_async_.wait()");
        cond_buffer_async_.wait(15 * 60 * 1000);
        __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::__AsyncLogThread() cond_buffer_async_ execute");
    }
}

void XloggerAppender::__WriteSync(const XLoggerInfo* _info, const char* _log) {
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::__WriteSync _info: %p, _log: [%s]", &_info, _log);
...
void XloggerAppender::__WriteAsync(const XLoggerInfo* _info, const char* _log) {
    __android_log_print(ANDROID_LOG_INFO, "zyp-xlog", "XloggerAppender::__WriteAsync _info: %p, _log: %s", &_info, _log);
    char temp[16 * 1024] = {0};  // tell perry,ray if you want modify size.
    ...
    if (nullptr == log_buff_)
        return;
    __android_log_print(ANDROID_LOG_DEBUG, "zyp-xlog", "LogBaseBuffer::__WriteAsync mmap length: %zu, log length: %d, ptr: %p, isNotifyAll: %d, log content: [%s]", log_buff_->GetData().Length(), (unsigned int)log_buff.Length(), log_buff_->GetData().Ptr(), (log_buff_->GetData().Length() + (unsigned int)log_buff.Length()) >= kBufferBlockLength * 1 / 3, _log);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "zyp-xlog", "LogBaseBuffer::__WriteAsync mmap write log before length: %d, ptr: %p", (unsigned int)log_buff.Length(), log_buff_->GetData().Ptr());
    if (!log_buff_->Write(log_buff.Ptr(), (unsigned int)log_buff.Length()))
        return;
    __android_log_print(ANDROID_LOG_DEBUG, "zyp-xlog", "LogBaseBuffer::__WriteAsync mmap write log after length: %d, ptr: %p", (unsigned int)log_buff.Length(), log_buff_->GetData().Ptr());
    if (log_buff_->GetData().Length() >= kBufferBlockLength * 1 / 3
        || (nullptr != _info && kLevelFatal == _info->level)) {
        __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "LogBaseBuffer::__WriteAsync mmap notifyAll() length: %zu", log_buff_->GetData().Length());
        cond_buffer_async_.notifyAll();
    }
}
...
void XloggerAppender::TreatMappingAsFileAndFlush(TFileIOAction* _result) {
    ...
    if (config_.compress_mode_ == kZstd) {
        ...
        log_buff_ = new LogZlibBuffer(data.release(), kBufferBlockLength, IS_COMPRESS, config_.pub_key_.c_str());
    }
    ...
    AutoBuffer buffer;
    __android_log_print(ANDROID_LOG_WARN, "zyp-xlog", "XloggerAppender::TreatMappingAsFileAndFlush()");
    log_buff_->Flush(buffer);
